...
function Addtocart() {
    ...
    // Create an array to store the items in the desired order 
    var items = [
      $('.add-to-cart-button-cuadro'),
      $('.add-to-cart-grupo'),
      $('.add-to-cart-llantas'),
      $('.add-to-cart-button-manubrio'),
      $('.add-to-cart-button-7')
    ];
  
    if (baseFinal2 == 0) {
      items.push($('.add-to-cart-acabados-b'));
    } else {
      items.push($('.add-to-cart-acabados-s'));
    }
  
    // Helper function to trigger click event on each item in order
    function triggerClick(index) {
      if (index < items.length) {
        items[index].trigger('click');
        setTimeout(function () {
          triggerClick(index + 1);
        }, 100); // Delay between triggering click events (adjust as needed)
      }
    }
  
    // Start triggering click events from the first item
    triggerClick(0);
};
