...
function addItemToCart(item) {
  ...
  function addSelectedItemsToCart() {
    ...
    // Create a promise for each item and add it to an array
    var promises = items.map(function (item) {
      return addItemToCart(item);
    });
  
    // Add the baseFinal2 item only if the first 5 items are present
    Promise.all(promises).then(function () {
      if (items.length === 5) {
        if (baseFinal2 == 0) {
          addItemToCart($('.add-to-cart-acabados-b')[0]).then(function () {
            $('#cart_wrapper2').fadeIn();
          });
        } else {
          addItemToCart($('.add-to-cart-acabados-s')[0]).then(function () {
            $('#cart_wrapper2').fadeIn();
          });
        }
      } else {
