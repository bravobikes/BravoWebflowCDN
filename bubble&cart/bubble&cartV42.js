...
function Addtocart() {
    ...
    items.push(getAddToCartButtonById('add-to-cart-button-cuadro'));
    items.push(getAddToCartButtonById('add-to-cart-grupo'));
    items.push(getAddToCartButtonById('add-to-cart-llantas'));
    items.push(getAddToCartButtonById('add-to-cart-button-manubrio'));
    items.push(getAddToCartButtonById('add-to-cart-button-7'));
  
    if (baseFinal2 == 0) {
      items.push(getAddToCartButtonById('add-to-cart-acabados-b'));
    } else {
      items.push(getAddToCartButtonById('add-to-cart-acabados-s'));
    }
      ...
      return new Promise(function(resolve) {
        if (item) {
          var observer = new MutationObserver(function(mutationsList, observer) {
            var cartItems = $("[data-wf-cart-action=remove-item]");
            if (cartItems.length > 0 && cartItems.is(':visible')) {
              resolve();
            }
          });
  
          observer.observe(document.getElementById('cart_wrapper2'), { childList: true });
  
          item.click();
        } else {
          console.error('Add-to-cart button not found:', item);
          resolve(); // Skip the item and continue
        }
      });
    });
  ...
  function getAddToCartButtonById(id) {
    var button = document.getElementById(id);
    if (button) {
      console.log('Add-to-cart button found:', button);
    } else {
      console.log('Add-to-cart button not found with ID:', id);
    }
    return button;
  }
