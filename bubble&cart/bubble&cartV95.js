...
LIST_OF_SIZEBIKE.forEach((e) => {
    e.addEventListener('change', (event) => {
        if (event.target.checked) {
            ...
            setCookie('bikeSize', CuadroRuta, 7); // 7 days cookie expiration
        }   
    });
});
...
LIST_OF_BIKETYPE.forEach((e) => {
    e.addEventListener('change', (event) => {
        ...
        setCookie('bikeStyle', CuadroType, 7); // 7 days cookie expiration 
    });
});

// FRAMESET ONLY : Yes or No
...
LIST_OF_GROUPSET.forEach( (e) => {
    e.addEventListener('change', (event) => {
        ...
        setCookie('groupset', groupsetFinal2, 7); // 7 days cookie expiration       
    });
});
...
// MANUBRIO : Aluminio y Carbon Aero
 LIST_OF_MANUBRIO.forEach( (e) => {
    e.addEventListener('change', (event) => {
        ...
        setCookie('manubrio', manubrioFinal2, 7);   
    });
});
...
window.addEventListener('load', () => {
    const savedBikeStyle = getCookie('bikeStyle');
    const savedGroupset = getCookie('groupset');
    const savedBikeSize = getCookie('bikeSize');
    const savedRines = getCookie('rines');
    const savedManubrio = getCookie('manubrio');
    const savedBase = getCookie('base');
    const savedDiseno = getCookie('diseno');
    const savedColor = getCookie('color');
    const savedEngraving = getCookie('grabado');

    if (savedBikeStyle) {
        var bikeStyleElement = document.getElementById(savedBikeStyle);
        if (bikeStyleElement) {
            bikeStyleElement.click();
        }
    }
    if (savedGroupset) {
        var groupsetElement = document.querySelector('input[name="groupset"][value="' + savedGroupset + '"]');
        if (groupsetElement) {
            groupsetElement.click();
        }
    }
    if (savedBikeSize) {
        var bikeSizeElement = document.getElementById(savedBikeSize);
        if (bikeSizeElement) {
            bikeSizeElement.click();
        }
        
    }
    if (savedRines) {
        var rinesElement = document.querySelector('input[name="rines"][value="' + savedRines + '"]');
        if (rinesElement) {
            rinesElement.click();
        }
    }
    if (savedManubrio) { 
        var manubrioElement = document.querySelector('input[name="manubrio"][value="' + savedManubrio + '"]');
        if (manubrioElement) {
            manubrioElement.click();
        }
    }
    if (savedBase) {
        var baseElement = document.querySelector('input[name="base"][value="' + savedBase + '"]');
        if (baseElement) {
            baseElement.click();
        }
    }
    if (savedDiseno) {
        var disenoElement = document.querySelector('input[name="diseno"][value="' + savedDiseno + '"]');
        if (disenoElement) {
            disenoElement.click();
        }
    }
    if (savedColor) {
        $('[value="' + savedColor + '"]').trigger('click');
    }
    if (savedEngraving === 'true') {
        $('[value="Yes"]').trigger('click');
    } else {
        $('[value="No"]').trigger('click');
    }
});
...
function Addtocart() {
    ...
    setCookie('selectedItems', JSON.stringify(items), 7);
