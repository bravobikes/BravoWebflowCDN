...
function saveCartToCookie(cartData) {
    ...
    Cookies.set('cart', cartDataString, { expires: 7 }); // Expires in 7 days
}

// Function to get cart data from the cookie
function getCartFromCookie() {
    try {
        const cartData = Cookies.get('cart'); // Corrected cookie key
        if (cartData) {
            return JSON.parse(cartData);
        }
    } catch (error) {
        // Handle JSON parsing error (e.g., invalid JSON data in the cookie)
        console.error('Error parsing cart data from the cookie:', error);
    }
    return null; // Return null if there was an issue with the cookie data
}
