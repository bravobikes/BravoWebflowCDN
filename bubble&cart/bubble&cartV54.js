...
function addSelectedItemsToCart() {
    ...
    // Create a promise to track when all items are added to the cart
    var promise = new Promise(function (resolve, reject) {
      ...
      items.forEach(function (item) {
        ...
        // Observe changes in the cart wrapper to check if the item is added
        var observer = new MutationObserver(function (mutationsList, observer) {
          for (var mutation of mutationsList) {
            // Check if the item is added to the cart
            if (
              mutation.target.classList.contains('w-commerce-commerceaddtocartmodalcartwrapper') &&
              mutation.addedNodes.length > 0
            ) {
              count++;
  
              // Check if all items are added to the cart
        ...
        observer.observe(document.getElementById('cart_wrapper2'), {
          childList: true, // Observe changes in the child nodes of the cart wrapper
        });
      });
    });
  
    // Open the cart wrapper with a delay after all items are added to the cart
    promise.then(function () {
      setTimeout(function () {
        $('#cart_wrapper2').fadeIn();
        clearPreviouslyAddedItems(); // Clear the previously added items from the cart
      }, 1000); // Adjust the delay as needed
    });
  }
  
  // Function to remove previously added items from the cart
  function clearPreviouslyAddedItems() {
