...
//this function updates the images after user selects a new wheel, groupset, or handlebar
    ...
    if (rines == 1){
        ...
        ruedos2Mobile = BICIS_URLS[('110001000NA000')]

        if (manubrio==1){
            manubrioImg1.src = BICIS_URLS[('220000100NA0001')]
            manubrioImgMobile1 = BICIS_URLS[('220000100NA0001')]
        }

        else if (manubrio==0){
            manubrioImg1.src = BICIS_URLS[('220000100NA000')]
            manubrioImgMobile = BICIS_URLS[('220000100NA000')]
        }

        if (groupset == 1){
            grupo1.src =  BICIS_URLS[('500010100NA000')]
            grupoMobile1 = BICIS_URLS[('500010100NA000')]
        }
        else if (groupset == 0){
            grupo1.src =  BICIS_URLS[('500000100NA000')]
            grupoMobile1 = BICIS_URLS[('500000100NA000')]
        }
    }

    else if (rines == 0){
        ruedos2.src = BICIS_URLS[('120000000NA000')]
        ruedos2Mobile = BICIS_URLS[('120000000NA000')]

        if (groupset == 1){  
            grupo1.src = BICIS_URLS[('500001100NA001')]
            grupoMobile1 = BICIS_URLS[('500001100NA001')]
        }

        else if (groupset == 0){
            grupo1.src = BICIS_URLS[('500000100NA001')]
            grupoMobile1 = BICIS_URLS[('500000100NA001')]
        }

        if (manubrio == 1){
            manubrioImg1.src = BICIS_URLS[('210000100NA0001')]
            manubrioImgMobile = BICIS_URLS[('210000100NA0001')]
        }

        else if (manubrio == 0){
            manubrioImg1.src =BICIS_URLS[('210000100NA000')]
            manubrioImgMobile = BICIS_URLS[('210000100NA000')]
        }
    }
}